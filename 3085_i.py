...
for i in range(n):
    for j in range(n - 1):
        ...
        if array[i][j] != array[i][j + 1]:
            array[i][j], array[i][j + 1] = array[i][j + 1], array[i][j]#switch 한꺼번에
            width()
            height()
            array[i][j + 1], array[i][j] = array[i][j], array[i][j + 1]#되돌리기
        
        # 만약 입력 받은 배열의 열의 원소가 다르다면
        if array[j][i] != array[j + 1][i]:
            array[j][i], array[j + 1][i] = array[j + 1][i], array[j][i]#switch
            width()
            height()
            array[j + 1][i], array[j][i] = array[j][i], array[j + 1][i]#되돌리기
